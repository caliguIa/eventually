...
use objc2_app_kit::{NSMenuItem, NSWorkspace, NSStatusItem};
...
impl MenuDelegate {
    pub fn new(mtm: MainThreadMarker, dismissed_events: Arc<Mutex<HashSet<String>>>, event_store: Retained<EKEventStore>, status_item: Retained<NSStatusItem>) -> Retained<Self> {
        let this = mtm.alloc();
        let this = this.set_ivars(MenuDelegateIvars { dismissed_events, mtm, event_store, status_item });
